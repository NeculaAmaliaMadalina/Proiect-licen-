...
import ReturTable from "./retur_table";
import { useNavigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import {
  returByPaginate,
  returRemove,
} from "../../../store/actions/retur.action";
...
const AdminRetur = (props) => {
  ...
  const goToEdit = (id) => {
    navigate(`/admin_dashboard/admin_returs/edit_retur/${id}`);
  };
  ...
  const handleRemove = () => {
    dispatch(returRemove(toRemove));
  };
  useEffect(() => {
    dispatch(returByPaginate(searchValues));
